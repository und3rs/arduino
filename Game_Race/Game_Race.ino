...
volatile long pressCountUser1 = 0;
volatile long pressCountUser2 = 0;
...
void runStartingEffect() {
  ...
  for(int i = 0; i < NUMPIXELS; i++) {
    tone(PIN_SPEAKER, startingMelody[i], DELAYVAL);
...
void onButton1Pressed() {
  ...
  if (interrupt_time - last_interrupt_time > 30) {
    ...
    Serial.println(pressCountUser1);
    #endif
  }
...
void onButton2Pressed() {
  ...
  if (interrupt_time - last_interrupt_time > 30) {
    ...
    Serial.println(pressCountUser2);
    #endif
  }
  last_interrupt_time = interrupt_time;
}
...
void loop() {
  switch(state) {
    ...
    case STATE_RUNNING: {
      int posUser1 = (pressCountUser1 / 2);
      int user1Finished = posUser1 > (5 * NUMPIXELS);

      int posUser2 = (pressCountUser2 / 2);
      int user2Finished = posUser2 > (5 * NUMPIXELS);

      int ledIndexOfUser1 = posUser1 % NUMPIXELS;
      int ledIndexOfUser2 = posUser2 % NUMPIXELS;

      pixels.clear();
      if(pressCountUser1 < pressCountUser2) {
        pixels.setPixelColor(ledIndexOfUser1, colorOfUser1);
        pixels.setPixelColor(ledIndexOfUser2, colorOfUser2);
      } else {
        pixels.setPixelColor(ledIndexOfUser2, colorOfUser2);
        pixels.setPixelColor(ledIndexOfUser1, colorOfUser1);
      }
      ...
      if(user1Finished && user2Finished) {
        ...
        #ifdef DEBUGGING
        Serial.println("++ FINISHED: User2 Win.");
        #endif
        state = STATE_END;
        winner = 2;
      }
